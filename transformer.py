...
class PositionalEncodingStoch(nn.Module):

    def __init__(self, max_len, d_model, dropout=0.1):
        super().__init__()
        self.dropout = nn.Dropout(dropout)
        self.pe = nn.Parameter(torch.zeros(1, 10*max_len, d_model), requires_grad=True)
        nn.init.trunc_normal_(self.pe)
    ...
    def forward(self, input):
        ...
        """
        T = input.shape[1]
        random = torch.randint(high=T,size=(1,))[0]
        return self.dropout(input + self.pe[:, random:random+T])
