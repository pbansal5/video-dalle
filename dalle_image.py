...
class DALLE(nn.Module):
    ...
    def forward(self, image, tau, hard, beta_dvae, attns_normalize=True):
        ...
        forward_res = {'recon' : recon.view(int(B/T),T,C,H,W).clamp(0., 1.),
        'recon_transformer': recon_transformer.view(int(B/T),T,C,H,W).clamp(0., 1.),
        'kl_dvae' : kl_dvae, 'mse': mse, 'attns' : attns.view(int(B/T),T,self.num_slots,C,H,W)}
